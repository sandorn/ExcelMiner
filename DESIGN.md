...
| Excel 写入 | Route 2：zip+quick-xml 纯 Rust ZIP/XML 操作（~1100行），替代 umya-spreadsheet |
...
- **公式写入**：`XlsxWriter::set_formula()` / `set_formula_with_value()` 写入 Excel 公式（ZIP+XML 路线）
...
`ReportWriter::write_summary()` 使用 `XlsxWriter`（Route 2 ZIP+XML）修改模板，按引擎类型向指定单元格写入汇总数据和 AI 结果：

- **填写页**：A2=月份、A4=年份、A1 公式 `=A2&A3` 含缓存值
- **行业 Sheet**（保险类/商写类/酒店类）：汇总指标 + AI 板块分析
- **公司 Sheet**：经营报表数据（G2:R18）+ AI 公司分析（C61）
- **SST**：追加模式，保留模板原有条目防止数据污染
...
zip = "2"                       # xlsx ZIP 容器操作
quick-xml = "0.37"              # xlsx XML 元素操作
# umya-spreadsheet = "2.3"      # [已废弃] Route 2 替代方案，仅保留 error.rs From trait
