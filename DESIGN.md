...
3. **AI分析** — 调用DeepSeek API逐公司生成经营分析，含5维度质量评分+最多2次重试
...
│   │   ├── DataImport.tsx        # Step 2 — 数据汇总
│   │   ├── AIAnalysis.tsx        # Step 3 — AI分析
│   │   └── ReportExport.tsx      # Step 4 — 报表导出
...
```
/     → 重定向到 /setup
/setup    → ProjectSetup   (Step 1)
/import   → DataImport     (Step 2)
/analysis → AIAnalysis     (Step 3)
/export   → ReportExport   (Step 4)
```

全局布局：左侧步骤导航（4步骤指示器）+ 右侧工作区 + 底部步骤按钮。
...
`From` trait 自动转换：`std::io::Error` → `Io`，`calamine::XlsxError` → `Excel`，`toml::de::Error` → `Config`，`rust_xlsxwriter::XlsxError` → `Other`。
...
- **公式写入**：用 `rust_xlsxwriter` 的 `write_formula()` 写入 SUM 公式
...
分析流程：
```
遍历公司列表 → 构建用户消息（嵌入汇总数据）→ 调用 DeepSeek API
   → 解析响应 JSON → 质量评分（5维度）→
       score >= 8 → 保存结果
       score < 8   → 重试（最多2次，通过 QualityChecker）
```
...
| 维度 | 检测内容 | 默认业态检测 |
|------|----------|------------|
| `has_summary` | 首行是否为无关键词摘要 | 通用 |
| `has_revenue` | d1维度（如"营业收入"） | 保险:人力/承保、酒店:营销/OTA、商写:整体/合作 |
| `has_ebitda` | d2维度（如"EBITDA"） | 保险:保费规模、酒店:评价评分、商写:渠道 |
| `has_cashflow` | d3维度（如"现金流"） | 保险:新单、酒店:入住率、商写:招商 |
| `has_expense` | d4维度（如"经营支出"） | 保险:无、酒店:无、商写:续租 |
...
`ReportWriter::write()` 使用 `rust_xlsxwriter` 0.80 生成多 sheet 的 xlsx：
...
子公司 .xlsx → calamine(ExcelReader<RS>) → NumberParser(文本→数字)
   → AggregationEngine(4引擎) → HashMap<公司, HashMap<指标, 值>>
        ...
        └── AIAnalyzer → DeepSeek API → AnalysisQuality(5维评分，满分10)
              ├── score>=8 → 保存结果
              └── score<8 → 重试(≤2次)
...
```json
{
    ...
    "permissions": [
        ...
        "dialog:allow-ask", "dialog:allow-confirm", "dialog:allow-message",
...
```json
{
    ...
    "bundle": {
        "windows": { "wix": { "language": "zh-CN" }, "nsis": { "languages": ["SimpChinese", "English"] } }
    }
}
```
...
| Phase 3 | DeepSeek API客户端、提示词加载、逐公司分析+重试、5维度质量评分 | ✅ |
| Phase 4 | xlsx报表写入、PPT文案导出、QualityChecker集成、页面联调 | ✅（主题样式待完善） |
| Phase 5 | 便携版打包、集成测试、MSI安装包 | 🔄 进行中 |