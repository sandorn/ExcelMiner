...
| Max Tokens       | **1000**                 | **1500**                  | **4096**             | ⚠️ **差异**       |
| HTTP Timeout     | **30s**                  | **60s**                   | **120s**             | ⚠️ **差异**       |
| API Key 来源     | `~/.dskey` [EXCEL]段     | 同                        | `~/.dskey` + 可配置  | ✅ 一致            |

> Rust 的 timeout=120s 和 max_tokens=4096 均大于 VBA 的最大值（60s, 1500），对实际使用影响有限但参数不一致。
...
| 提示词内容           | `经营分析提示词.md` — 完整版（含利润负数/支出特殊规则） | `PROMPT_FINANCIAL` — 精简版                  | ⚠️ **差异**    |
| **批次大小**         | **3**                                                   | **3** (可配置)                               | ✅ 一致         |
| **批次延迟**         | **1000ms** (`Application.Wait`)                         | **未实现**                                   | ❌ **待补**     |
| **质量评分阈值**     | **≥8 分** (满分10)                                      | **≥4 分** (可配置，默认值)                   | 🔴 **重大差异** |
| **最大重试次数**     | **2 次**                                                | **3 次** (可配置)                            | ⚠️ 差异         |
| **重试延迟**         | **2 秒** (`RETRY_DELAY_MS=2000`)                        | **无显式延迟**（立即重试）                   | ⚠️ **差异**     |
| 质量未达标标记       | `[质量提示：本分析质量评分 X/10，部分指标描述可能不完整]` | `【质量不达标，评分 X/Y】`                    | ✅ 一致         |
| 质量评分维度         | 5维 (summary/revenue/ebitda/cashflow/expense)           | 5维 (相同)                                   | ✅ 一致         |
| HTTP层重试策略       | 指数退避 `2^retry` 秒，最多3次                          | 无指数退避（固定重试）                       | ⚠️ 差异         |
...
```rust
// Rust: quality_checker.rs
let score = (quality.has_summary as u8) * 2
    + (quality.has_revenue as u8) * 2
...
| # | 功能             | VBA 原型要求          | Rust 当前实现          | 影响                                      |
|---| ---------------- | --------------------- | ---------------------- | ----------------------------------------- |
| 1 | **质量评分阈值** | ≥8 分 (满分10)        | 默认 ≥4 分             | AI 质量管控大幅放宽，可能输出不达标分析   |
| 2 | **单元格格式**   | 设置 NumberFormat     | **完全未实现**          | 导出 xlsx 缺少格式，用户需手动设置       |
| 3 | **经营报表处理** | 纯值复制 D4:O20→G2:R18| 结构化JSON提取          | 输出数据布局完全不同                      |
| 4 | **提示词完整性** | 含利润负数/支出判断等 | 精简版，缺少专业规则    | AI分析质量可能下降，尤其是亏损公司       |

### 🟡 中等差异（P1 — 近期处理）

| # | 功能             | VBA 原型                        | Rust 当前实现                | 建议                        |
|---| ---------------- | ------------------------------- | ---------------------------- | --------------------------- |
| 5 | **批次延迟**     | 1000ms (`Application.Wait`)     | 无延迟                       | 补充 `tokio::time::sleep`  |
| 6 | **重试延迟**     | 2秒固定延迟                     | 无显式延迟（立即重试）       | 补充重试间隔               |
| 7 | **重试策略**     | HTTP层指数退避 `2^retry` 秒    | 无指数退避                   | 补充指数退避逻辑           |
| 8 | **API Timeout**  | 60s (公司分析) / 30s (板块分析) | 统一 120s                    | 统一为 60s 或提供配置项    |
| 9 | **Max Tokens**   | 1500 (公司分析) / 1000 (板块)   | 统一 4096                    | 统一为 2000 或提供配置项   |
...
1. **调整质量评分阈值为 8 分**
   - 位置：`src-tauri/src/config.rs` → `AIConfig.quality_threshold` 默认值
   - 修改：`4` → `8`
...
2. **补充 Excel 单元格格式设置**
   ...
   - 技术方案：`umya_spreadsheet` 的 `Style.NumberFormat`

3. **完善经营分析提示词**
   - 位置：`src-tauri/src/services/ai_analyzer.rs` → `PROMPT_FINANCIAL`
   - 需补充内容：利润类负值处理规则、支出判断的详细逻辑、输出格式的严格模板

### P1 — 近期实现

4. **实现批次间延迟**
   - 位置：`src-tauri/src/commands/analysis_cmd.rs`
   - 方案：在 `execute_company_analysis` 的公司循环中加入 `tokio::time::sleep(Duration::from_millis(1000))`

5. **补充重试间延迟**
   - 位置：`src-tauri/src/services/ai_analyzer.rs` → `analyze_single` 重试循环
   - 方案：失败后 `sleep(2000)` 再重试

6. **统一重试次数为 2 次**
   - 位置：`src-tauri/src/config.rs` → `AIConfig.max_retries` 默认值
   - 修改：`3` → `2`
...
7. **指数退避重试**
   ...
   - 方案：`delay = 2^retry * 1000ms`

8. **经营报表原始数据保留**
   - 考虑在 JSON 输出之外，同时保留原始的 16行×12列 布局数据，供 report_writer 直接还原到 `G2:R18`
   - 方案：在 `AggregationResult.summary_data` 中增加 `raw_grid` 字段

9. **统一 API 参数**
   - timeout: 60s（与 VBA 核心指标分析一致）
   - max_tokens: 1500（与 VBA 核心指标分析一致）
   - 或提供 UI 配置项让用户自行选择
